...
public class NoesisPostprocessor : AssetPostprocessor
{
    ...
    private static void ReloadTexture(string uri, Texture2D texture)
    {
        Log("ReloadTexture " + uri);

        // Unregister old destroyed texture and register the new imported one
        NoesisTextureProvider.instance.Unregister(uri);
        NoesisTextureProvider.instance.Register(uri, texture);

        // Notify of the change
        NoesisTextureProvider.instance.ReloadTexture(uri);
    }
